...
### Step 3 verification (completed 28 July 2026)

The `accounts` app was created:

```powershell
.\.venv\Scripts\python.exe manage.py startapp accounts
```

It was registered in `config/settings.py` under `INSTALLED_APPS`. The models are:

- **Household**: `name`, `base_currency` (default GBP), `fiscal_year_start_month` (default 4/April), `created_at`. One household isolates all financial data — every downstream model gets a `household` FK.
- **UserProfile**: `user` (OneToOneField to `User`), `household` (FK to `Household`), `display_name`, `descriptive_role` (Primary User / Secondary User / Joint), `permission_role` (Admin / Member / Viewer), `avatar_url`. The `permission_role` governs access; `descriptive_role` is a display label for attribution.

Migration created `accounts_household` and `accounts_userprofile` tables. Both models registered in `accounts/admin.py` with list displays, filters, and search.
...
`manage.py check` confirms zero issues.

## Step 4: Household-scoping middleware and auth views

### Goal

Build middleware that stamps `request.household` on every authenticated request, then create the sign-up, login, logout, and invite views so users can create households and join them.

### What

The middleware reads `request.user.profile.household` and attaches it to the request object. Every downstream viewset uses `request.household` to scope its queryset, enforcing strict data isolation per household.

Auth views handle:

- **Sign-up**: creates a `User`, a `Household`, and a `UserProfile` in one transaction.
- **Login / Logout**: standard Django session auth, no JWT needed since the React SPA is served from the same origin.
- **Invite accept**: a logged-in user with a valid invite token is joined to the target household.

### Why

Without household-scoped middleware, every viewset would need to manually look up `request.user.profile.household`. Centralising it in middleware makes isolation the default, not an afterthought. Auth views are the entry point — nothing else works until users can sign up and belong to a household.
...
Then wire it into `config/settings.py` under `MIDDLEWARE`, after `AuthenticationMiddleware`.

Auth views live in `accounts/views.py` and are mapped in `accounts/urls.py`, included from `config/urls.py` under `api/auth/`.
