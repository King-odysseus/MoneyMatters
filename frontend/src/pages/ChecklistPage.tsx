...
export default function ChecklistPage() {
  ...
  const addItem = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["checklist"] }); setNewName(""); setNewAmount(""); },
  });
  ...
  const handleAdd = () => {
    ...
    addItem.mutate({ checklist_id: checklist.id, name: newName.trim(), projected_amount: newAmount || "0", display_order: checklist.items.length });
  };
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <div>
          <h2 className="text-2xl font-bold text-gray-100">Monthly Checklist</h2>
          <p className="text-sm text-gray-500 mt-0.5">{checklist?.month_label}</p>
        </div>
        <div className="flex gap-4 text-sm">
          <div className="text-center">
            <p className="text-xs text-gray-500 uppercase">Projected</p>
            <p className="font-semibold text-gray-300">GBP {totalProjected.toFixed(2)}</p>
          </div>
          <div className="text-center">
            <p className="text-xs text-gray-500 uppercase">Paid</p>
            <p className="font-semibold text-emerald-400">GBP {totalPaid.toFixed(2)}</p>
          </div>
          <div className="text-center">
            <p className="text-xs text-gray-500 uppercase">Remaining</p>
            <p className="font-semibold text-bronze-400">{remaining} items</p>
          </div>
        </div>
      </div>
      ...
      <div className="card overflow-hidden">
        {checklist?.items.length === 0 ? (
          <div className="p-8 text-center text-gray-500 text-sm">No items yet. Add your first bill above.</div>
        ) : (
          <div className="divide-y divide-navy-500">
            {checklist?.items.map((item) => {
              ...
              return (
                <div key={item.id} className={`flex items-center gap-4 px-5 py-3.5 transition-colors ${item.is_paid ? "bg-navy-800/50" : "hover:bg-navy-800/30"}`}>
                  <button onClick={() => togglePaid.mutate({ id: item.id })}
                    className={`w-5 h-5 rounded-md border-2 flex items-center justify-center shrink-0 transition-colors ${
                      item.is_paid ? "bg-emerald-500 border-emerald-500 text-white" : "border-navy-400 hover:border-primary-400"
                    }`}>
                    {item.is_paid && (
                  ...
                  <div className="flex-1 min-w-0">
                    <p className={`text-sm font-medium ${item.is_paid ? "text-gray-500 line-through" : "text-gray-200"}`}>{item.name}</p>
                    {item.notes && <p className="text-xs text-gray-500 mt-0.5">{item.notes}</p>}
                  </div>
                  <div className="text-right shrink-0 w-24">
                    {editItemId === item.id ? (
                      <input type="number" step="0.01" defaultValue={actual}
                        onChange={(e) => setEditAmount(e.target.value)}
                        className="w-20 px-2 py-1 bg-navy-800 border border-navy-400 rounded text-right text-sm text-gray-200 focus:outline-none focus:ring-1 focus:ring-primary-500"
                        autoFocus
                        onKeyDown={(e) => {
                      ...
                      <div>
                        <p className={`text-sm font-semibold ${item.is_paid ? "text-gray-500" : "text-gray-200"}`}>
                          GBP {parseFloat(actual).toFixed(2)}
                        </p>
                        {hasAdjustment && (
                          <p className={`text-xs ${diff > 0 ? "text-red-400" : "text-emerald-400"}`}>
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    {!item.is_paid && (
                      <button onClick={() => { setEditItemId(item.id); setEditAmount(actual); }}
                        className="p-1.5 text-gray-500 hover:text-primary-400 hover:bg-navy-800 rounded-lg transition-colors" title="Adjust amount">
                    ...
                    <button onClick={() => deleteItem.mutate(item.id)}
                      className="p-1.5 text-gray-500 hover:text-red-400 hover:bg-navy-800 rounded-lg transition-colors" title="Remove">
                      <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
